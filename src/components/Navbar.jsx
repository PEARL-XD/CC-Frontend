...
export default function Navbar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (res.ok) { setAccessToken(null); navigate("/login"); }
    } catch (err) {
  ...
  const clearSearch = useCallback(() => {
    setQuery(""); setResults([]); setOpen(false); setHighlightIndex(-1);
  }, []);
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setHighlightIndex((i) => Math.min(i + 1, results.length - 1)); }
      ...
      if (item) { navigate(`/product/${item._id || item.id}`); clearSearch(); }
  ...
  return (
    <>
      {/* ── Main navbar ── */}
      <nav style={{
        width: "100%", background: BG, position: "sticky", top: 0, zIndex: 50,
  ...
  />
          ...
          <div className="cc-search" style={{ position: "relative" }}>
            <label htmlFor="nav-search" className="sr-only">Search items</label>
            <svg
              style={{ position: "absolute", left: "13px", top: "50%", transform: "translateY(-50%)", width: "15px", height: "15px", pointerEvents: "none" }}
              viewBox="0 0 24 24" fill="none" stroke="#bbb" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            >
            ...
            <div style={{ position: "absolute", right: "13px", top: "50%", transform: "translateY(-50%)" }}>
              {loading ? (
                <svg style={{ width: "15px", height: "15px", animation: "cc-spin 0.8s linear infinite", color: "#ccc" }} viewBox="0 0 24 24">
                ...
                <button
                  ...
                  onClick={clearSearch}
                  style={{ background: "none", border: "none", cursor: "pointer", color: "#bbb", fontSize: "15px", lineHeight: 1, padding: "2px", display: "flex" }}
                >
                  ✕
                </button>
              ) : null}
            ...
            {open && results.length > 0 && (
              <ul
                ...
                style={{
                  ...
                  listStyle: "none", padding: "6px", margin: 0, zIndex: 60,
                }}
              >
                {results.map((item, idx) => (
                  <li
                    ...
                    style={{
                      ...
                      background: idx === highlightIndex ? "#fdf8f5" : "transparent",
                      transition: "background 0.1s",
                    }}
                  >
                    <img
                      src={item.img} alt={item.name}
                      style={{ width: "44px", height: "44px", objectFit: "cover", borderRadius: "8px", flexShrink: 0, border: `1px solid ${BORDER}` }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: "13.5px", fontWeight: 600, color: "#1a1a1a", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.name}</div>
                      {item.desc && <div style={{ fontSize: "11.5px", color: "#aaa", marginTop: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.desc}</div>}
                    </div>
                    <div style={{ fontSize: "13px", fontWeight: 700, color: RED, flexShrink: 0 }}>₹{item.price}</div>
                  </li>
                ))}
                <li style={{ textAlign: "center", fontSize: "11px", color: "#ccc", padding: "6px 0 2px" }}>
            ...
            {open && !loading && results.length === 0 && (
              <div style={{
                ...
                padding: "1rem", textAlign: "center", fontSize: "13px", color: "#bbb", zIndex: 60,
              }}>
                No items found
              </div>
            )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            >
              <svg style={{ width: "19px", height: "19px" }} viewBox="0 0 24 24" fill="none" stroke={cartCount > 0 ? RED : "#bbb"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div style={{ position: "relative" }} ref={userMenuRef}>
              <button
                ...
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#f0b8a8"; e.currentTarget.style.boxShadow = "0 2px 8px rgba(229,100,57,0.10)"; }}
              ...
              >
                ...
                <svg
                  style={{ width: "13px", height: "13px", color: TEXT_MID, transform: userDropdownOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}
                ...
                <div style={{
                  ...
                  {user?.name && (
                    <div style={{ padding: "12px 14px 10px", borderBottom: `1px solid ${BORDER}` }}>
                      ...
                      <div style={{ fontSize: "11.5px", color: "#bbb", marginTop: "2px" }}>{user.phone}</div>
                    </div>
                  )}
                  <div style={{ padding: "6px" }}>
                    {isAdmin && <DropItem to="/admin/orders" label="Admin Panel" icon="⚡" accent={RED} onClick={() => setUserDropdownOpen(false)} />}
                    <DropItem to="/profile"    label="Profile"          icon="👤" onClick={() => setUserDropdownOpen(false)} />
                    <DropItem to="/orderspage" label="My Orders"        icon="📦" onClick={() => setUserDropdownOpen(false)} />
                    <DropItem to="/support"    label="Support"          icon="💬" onClick={() => setUserDropdownOpen(false)} />
                    <DropItem to="/info"       label="About & Policies" icon="📄" onClick={() => setUserDropdownOpen(false)} />
                    <div style={{ borderTop: `1px solid ${BORDER}`, marginTop: "4px", paddingTop: "4px" }}>
                      <button
                        ...
                        onMouseEnter={(e) => { e.currentTarget.style.background = "#fdf8f5"; e.currentTarget.style.color = "#E53935"; }}
  ...
  return (
    <button
      ...
      style={{
        display: "flex", alignItems: "center", gap: "8px",
        width: "100%", padding: "8px 10px", borderRadius: "8px",
        background: hov ? "#fdf8f5" : "none", border: "none", cursor: "pointer",
        fontSize: "13px", color: accent ?? (hov ? "#1a1a1a" : "#888"),
